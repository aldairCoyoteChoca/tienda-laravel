...
<div class="container">
  <ul class="nav nav-pills mb-12" id="pills-tab" role="tablist">
    ...
    <li class="nav-item">
      <a class="nav-link" id="pills-devueltos-tab" data-toggle="pill" href="#pills-devueltos" role="tab" aria-controls="pills-devueltos" aria-selected="false">Devoluciones</a>
    </li>
  </ul>
  <div class="tab-content" id="pills-tabContent">
    ...
    <div class="tab-pane fade show active" id="pills-encamino" role="tabpanel" aria-labelledby="pills-encamino-tab">
      ...
      <div class="card text-center">
        ...
        <div class="card-footer text-muted">
          <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#modalsucces{{$cart->id}}"><i class="fas fa-check-circle"></i> Confirmar pedido</button>
      ...
      <div class="modal fade" id="modalsucces{{$cart->id}}" tabindex="-1" role="dialog" aria-labelledby="modalsuccesTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header mx-auto">
                    <h5 class="modal-title mx-auto" id="modalsuccesTitle">Pedido "{{ config('app.name') }} {{$cart->id}}" </h5>
                </div>
                <div class="modal-body mx-auto">
                    ¿Estás seguro de querer confirmar como entregado este pedido?</h6>
                </div>
                <div class="mx-auto">
                    <small>Esta acción no puede deshacerse.</small>
                </div>
                <div class="mx-auto">
                  <small>Se enviara un correo de confirmación.</small>
              </div>
                <div class="modal-footer mx-auto">
                    <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">Cancelar</button>
                    <form method="POST" action="{{ route('entregado') }}">
                      @csrf
                      <input type="hidden" name="id" value="{{$cart->id}}">
                      <button class="btn btn-sm btn-success text-white"><i class="fas fa-check-circle"></i> Confirmar
                      </button>
                    </form>
                </div>
            </div>
    ...
    <div class="tab-pane fade" id="pills-entregados" role="tabpanel" aria-labelledby="pills-entregados-tab">
      <div id="test-swipe-2" class="col s10">
        ...
        <div class="card text-center">
          ...
          <div class="card-footer text-muted">
            <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#modalDevolucion{{$cart->id}}"><i class="fas fa-sad-tear"></i> Devolver pedido</button>
          </div>
        </div>
        <div class="modal fade" id="modalDevolucion{{$cart->id}}" tabindex="-1" role="dialog" aria-labelledby="modalDevolucionTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
                  <div class="modal-header mx-auto">
                      <h5 class="modal-title mx-auto" id="modalDevolucionTitle">Pedido "{{ config('app.name') }}{{$cart->id}}" </h5>
                  </div>
                  <div class="modal-body mx-auto">
                      ¿Estás seguro de querer devolver este pedido?</h6>
                  </div>
                  <div class="mx-auto">
                      <small>Esta acción no puede deshacerse.</small>
                  </div>
                  <div class="modal-footer mx-auto">
                      <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">Cancelar</button>
                      <form method="POST" action="{{ route('devolucion') }}">
                        @csrf
                        <input type="hidden" name="id" value="{{$cart->id}}">
                        <button class="btn btn-sm btn-danger text-white"><i class="fas fa-sad-tear"></i></i> Devolver
                        </button>
                      </form>
                  </div>
              </div>
    ...
    {{-- pedidos devueltos --}}
    <div class="tab-pane fade" id="pills-devueltos" role="tabpanel" aria-labelledby="pills-devueltos-tab">
      <div id="test-swipe-3" class="col s10">
        @forelse ($cartsDevueltos as $cart)
        <div class="card text-center">
          <div class="card-header">
            Pedido "{{ config('app.name') }} {{$cart->id}}" 
          </div>
      ...
      <br>
      <div class="container">
        <h5 class="text-center text-monospace">¡No tienes pedidos devueltos aún!</h5>
      </div>
      @endforelse
      </div>
    </div>
